################################################################################
#NAME:         0302_summary_results_archetypes.R
#AUTOR:        Alessa Widmaier and co-authors
#DATE:         03/05/2026
#DESCRIPTION:  Aggregate results by city archetype
#RUNNING TIME: <1min
################################################################################
...
cat(green("\n\nSummarising archetype results started."))
...
#Loop across archetypes and link to World Bank income groups
for(archetype in c("established_ageing", "maturing", "transitioning", "fast_growing")){
  
  if(archetype == "established_ageing"){
    income <- "High income"
  } else if(archetype == "maturing"){
    income <- "Upper Middle"
  } else if(archetype == "transitioning"){
    income <- "Lower Middle"
  } else {
    income <- "Low income"
  }
...
#2.1: Read archetype-specific model inputs -------------------------------------
  
  #Get list of cities within archetype in question
  archetypes <- readxl::read_excel(
    "01_data/01_raw/01_cities/GHSL_Cities_DB/ghsl_population_projection.xlsx") %>%
    filter(world_bank_income_group == income) %>%
    dplyr::select(city_id = unique_id, city_name = urban_centre_main_name) %>%
    distinct()

  
  
#2.2: Read health impact model outputs -----------------------------------------
  
  # load the heat death and daly counts
  heat_model <- read_rds("01_data/02_interim/05_heat/heat_health_impacts.rds") %>%
    mutate(risk = "Heat") %>%
    filter(city_id %in% archetypes$city_id, city_name %in% archetypes$city_name)
  
  air_qual_model <- read_rds("01_data/02_interim/04_air_quality/airqual_health_impacts.rds") %>%
    mutate(risk = "Air Quality") %>%
    filter(city_id %in% archetypes$city_id, city_name %in% archetypes$city_name)
...
#2.3: Read supporting datasets -------------------------------------------------
  ...
  ihme_totals_all_cause <- list.files(
    ...
    mutate(
      wb_country_code = suppressWarnings(countrycode::countrycode(
        location,
...
#3.1: Aggregate model outputs by archetype -------------------------------------
    ...
    summarise(
      ...
      across(
        c(
          ...
          #change_45_2040, change_85_2040,
          #change_45_2050, change_85_2050,
...
#5.1: Combine and save archetype summary tables --------------------------------
  
  combined_summary_table <- heat_summary_table |>
    select(risk, summary_variable, Deaths, DALYs) |>
    bind_rows(others_summary_table)
  
  write_csv(combined_summary_table, paste0("01_data/03_final/supplementary/", archetype, "_table_s1_alternative.csv"))
}


#Display completion message
cat(green("\nSummarising archetype results complete."))
